...
public class ControlView extends RelativeLayout implements View.OnTouchListener {
    ...
    private LinearLayout llDaoRu;
    private LinearLayout llHuiShan;
    private LinearLayout llWanCheng;
    private LinearLayout llDaojishi;
    private LinearLayout llHuanJIngTou;
    ...
    private void assignViews() {
        ...
        aliyunDelete =  findViewById(R.id.aliyun_delete);
        llBeautyFace = findViewById(R.id.ll_beauty_face);
        llGifEffect = findViewById(R.id.ll_gif_effect);
        llDaoRu = findViewById(R.id.ll_daoru);
        llHuiShan = findViewById(R.id.ll_huishan);
        llWanCheng = findViewById(R.id.ll_wancheng);
        llDaojishi=findViewById(R.id.alivc_record_ready_record);
        llHuanJIngTou=findViewById(R.id.switch_camera);
    ...
    private void setViewListener() {
        ...
        aliyunDelete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (mListener != null) {
                    ...
                    llDaoRu.setVisibility(VISIBLE);
                    llWanCheng.setVisibility(GONE);
                    llHuiShan.setVisibility(GONE);
                }
            }
        });
        // 点击动图
        ...
        llDaoRu.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (FastClickUtil.isFastClick()) {
                    return;
    ...
    private void updateTittleView() {
        if (recordState == RecordState.STOP) {
            ...
            llHuanJIngTou.setVisibility(VISIBLE);
            mLlFilterEffect.setVisibility(GONE);
            llDaojishi.setVisibility(VISIBLE);
            updateLightSwitchView();
            updateMusicSelView();
            updateCompleteView();
        } else {
            llHuanJIngTou.setVisibility(GONE);
            mTitleView.setVisibility(GONE);
            mAlivcMusic.setVisibility(GONE);
            mLlFilterEffect.setVisibility(GONE);
            llDaojishi.setVisibility(GONE);
        }
    }
...
//            UIConfigManager.setImageResourceConfig(aliyunComplete, R.attr.finishImageUnable, R.mipmap.alivc_svideo_icon_next_not_ready);
    ...
    private void updateMusicSelView() {
        if (hasRecordPiece) {
            ...
            mIvMusicIcon.setColorFilter(ContextCompat.getColor(getContext(), R.color.alivc_svideo_gray_alpha));
            mTvMusic.setTextColor(ContextCompat.getColor(getContext(), R.color.alivc_base_white));
//            mTvMusic.setTextColor(ContextCompat.getColor(getContext(), R.color.alivc_music_gray));
        } else {
            mIvMusicIcon.setColorFilter(ContextCompat.getColor(getContext(), R.color.alivc_svideo_gray_alpha));
//            mIvMusicIcon.setColorFilter(null);
            mTvMusic.setTextColor(ContextCompat.getColor(getContext(), R.color.alivc_base_white));
    ...
    private void updateBottomView() {
        if (isEffectSelViewShow) {
            ...
            if (recordState == RecordState.STOP) {
                ...
                llDaoRu.setVisibility(INVISIBLE);
                llWanCheng.setVisibility(GONE);
                llHuiShan.setVisibility(GONE);
            }
        }
    ...
    private void updateDeleteView() {
        ...
        if (!hasRecordPiece || recordState == RecordState.RECORDING
            ...
            llHuiShan.setVisibility(VISIBLE);
            if(canComplete){
                llWanCheng.setVisibility(VISIBLE);
            }
            else {
                llWanCheng.setVisibility(GONE);
            }
    ...
    public void setMusicIcon(String icon){
        new ImageLoaderImpl()
            ...
            .loadImage(getContext(), icon , new ImageLoaderOptions.Builder()
                .placeholder(R.mipmap.aliyun_svideo_music)
                .circle()
