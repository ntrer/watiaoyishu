...
public class TaGuanZhuActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener,BaseSectionQuickAdapter.RequestLoadMoreListener {
    ...
    private void shouData(final List<Test.DataListBean> data) {
        try {
            mTaGaunZhuAdapter = new TaGaunZhuAdapter(R.layout.item_tafensi, data);
            final LinearLayoutManager manager = new LinearLayoutManager(TaGuanZhuActivity.this);
            mRvRecycleview.setLayoutManager(manager);
            mRvRecycleview.setAdapter(mTaGaunZhuAdapter);
            mTaGaunZhuAdapter.setOnLoadMoreListener(this,mRvRecycleview);
        } catch (Exception e) {
            ToastUtils.showLong(e.toString());
        }
    }
    ...
    private void loadMore() {
        ...
        try {
            ...
            OkhttpUtil.okHttpPost(url, paramsMap, new CallBackUtil.CallBackString() {
                @Override
                public void onFailure(Call call, Exception e) {
                    mTaGaunZhuAdapter.loadMoreComplete();
                    mTaGaunZhuAdapter.loadMoreEnd();
                }
    ...
    private void LoadMoreData(List<Test.DataListBean> data) {
        if (data.size() != 0) {
            mTaGaunZhuAdapter.addData(data);
            mTaGaunZhuAdapter.loadMoreComplete();
        }
