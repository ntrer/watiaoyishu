...
public class TaDeFenSiActivity extends BaseActivity implements  SwipeRefreshLayout.OnRefreshListener,BaseSectionQuickAdapter.RequestLoadMoreListener {
    ...
    private void shouData(final List<Test.DataListBean> data) {
        try {
            mTaFenSiAdapter = new TaFenSiAdapter(R.layout.item_tafensi, data);
            final LinearLayoutManager manager = new LinearLayoutManager(TaDeFenSiActivity.this);
            mRvRecycleview.setLayoutManager(manager);
            mRvRecycleview.setAdapter(mTaFenSiAdapter);
            mTaFenSiAdapter.setOnLoadMoreListener(this,mRvRecycleview);
        } catch (Exception e) {
            ToastUtils.showLong(e.toString());
        }
    }
    ...
    private void loadMore() {
        ...
        try {
            ...
            OkhttpUtil.okHttpPost(url, paramsMap, new CallBackUtil.CallBackString() {
                @Override
                public void onFailure(Call call, Exception e) {
                    mTaFenSiAdapter.loadMoreComplete();
                    mTaFenSiAdapter.loadMoreEnd();
                }
