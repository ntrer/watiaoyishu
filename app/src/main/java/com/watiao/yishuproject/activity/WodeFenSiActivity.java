...
public class WodeFenSiActivity extends BaseActivity implements  SwipeRefreshLayout.OnRefreshListener,BaseSectionQuickAdapter.RequestLoadMoreListener{
    ...
    private void shouData(final List<Test.DataListBean> data) {
        try {
            mWoFenSiAdapter = new WoFenSiAdapter(R.layout.item_wofensi, data);
            final LinearLayoutManager manager = new LinearLayoutManager(WodeFenSiActivity.this);
            mRvRecycleview.setLayoutManager(manager);
            mRvRecycleview.setAdapter(mWoFenSiAdapter);
            mWoFenSiAdapter.setOnLoadMoreListener(this,mRvRecycleview);
        } catch (Exception e) {
            ToastUtils.showLong(e.toString());
        }
    }
    ...
    private void loadMore() {
        ...
        try {
            ...
            OkhttpUtil.okHttpPost(url, paramsMap, new CallBackUtil.CallBackString() {
                @Override
                public void onFailure(Call call, Exception e) {
                    mWoFenSiAdapter.loadMoreComplete();
                    mWoFenSiAdapter.loadMoreEnd();
                }
