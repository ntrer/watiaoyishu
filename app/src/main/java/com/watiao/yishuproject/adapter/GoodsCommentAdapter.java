...
public class GoodsCommentAdapter extends BaseQuickAdapter<Test.DataListBean,BaseViewHolder> {
    ...
    public GoodsCommentAdapter(int layoutResId, @Nullable List<Test.DataListBean> data) {
    ...
    protected void convert(BaseViewHolder helper, Test.DataListBean item) {
        try {
            helper.setText(R.id.name,"汉兰达");
            view2= helper.getView(R.id.avatar);
            Glide.with(mContext).load(avatar[mRandom.nextInt(avatar.length)]).placeholder(R.mipmap.touxiang_ph).into(view2);
        }
        catch (Exception e){
