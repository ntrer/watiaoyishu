...
public class RiQiAdapter extends BaseQuickAdapter<Test.DataListBean,BaseViewHolder> {
    ...
    public RiQiAdapter(int layoutResId, @Nullable List<Test.DataListBean> data) {
    ...
    protected void convert(BaseViewHolder helper, Test.DataListBean item) {
        try {
            helper.setText(R.id.zuibiao_dizhi,"北京");
            view = helper.getView(R.id.saishi_pic);
            Glide.with(mContext).load(avatar[mRandom.nextInt(avatar.length)]).into(view);
            if(helper.getAdapterPosition()==0){
                helper.getView(R.id.content).setBackground(mContext.getResources().getDrawable(R.drawable.layout_border21));
            }
        }
        catch (Exception e){
